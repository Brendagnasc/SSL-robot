...
RobotCmd KickToGoal::execute(int robot_id, WorldModel& world) {
    ...
    if (kicked_) {
        ...
        if (ms > COOLDOWN_MS) {
            ...
            // Durante cooldown volta para posição de apoio atrás da bola
            float support_x = ball->x - 500.0f;
            float support_y = ball->y;
    ...
    if (dist_ball < KICK_DISTANCE) {
        ...
        kicked_ = true;
        last_kick_ = now;
        std::cout << "[KickToGoal] Robô " << robot_id << " CHUTOU!\n";
    } else {
        float dx_goal = goal_x_ - ball->x;
        float dy_goal = goal_y_ - ball->y;
        float d = std::sqrt(dx_goal*dx_goal + dy_goal*dy_goal);

        float approach_x = ball->x - (dx_goal / d) * 250.0f;
        float approach_y = ball->y - (dy_goal / d) * 250.0f;
